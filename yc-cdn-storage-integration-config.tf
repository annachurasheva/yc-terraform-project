...
provider "yandex" {
  service_account_key_file = "terraform-sa-key.json"  # Путь к вашему файлу с ключом
  cloud_id                 = "b1gcf9hqbk2k51drjuep"   # Например, из переменной var.cloud_id
  folder_id                = "b1gvfa1a7c0iuksv5g5t" # Например, из переменной var.folder_id
  zone                     = "ru-central1-a"
}
data "yandex_cm_certificate" "existing-cert" {
  certificate_id = "fpqbp2l227tqc798uccg"
}
...
resource "yandex_cdn_resource" "my-resource" {
  ...
  ssl_certificate {
    ...
    certificate_manager_id = data.yandex_cm_certificate.existing-cert.id
  }
  options {
...
output "cdn_cname" {
  description = "CNAME для CDN-ресурса (нужно добавить в CloudFlare)"
  value       = yandex_cdn_resource.my-resource.provider_cname
}